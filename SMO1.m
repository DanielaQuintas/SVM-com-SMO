    ...
    alpha=[];
    %ler o data set 
    Data=csvread('dataset1.csv');   %48 pontos usar Kernel linear!
...
%O MESMO PROCESSO CONSIDERANDO C=1000
...
%     
   ...
   fprintf('\n out-sample error: %.4e ', ErrDv);
    ...
    contour(x1Grid,x2Grid,reshape(Yp,size(x1Grid)),[0 0], 'k','LineColor', 'red');  %'ShowText','on'
    title('SVM dual com SMO  + fronteira de decisão');
    legend('-1:Outras Espécies-Treino','+1:Iris-Setosa-Treino','Vetores Suporte','-1:Outras Espécies-validação','+1:Iris-Setosa-validação',"C=10","C=1000")
    %----
